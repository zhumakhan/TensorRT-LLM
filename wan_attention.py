...
kv_packed = torch.randn(1, 257, 2, 40 * 128, dtype=torch.float16, device='cuda')

cu_seqlen_q = torch.tensor([0, 49392], dtype=torch.int32, device='cuda')
cu_seqlen_kv = torch.tensor([0, 257], dtype=torch.int32, device='cuda')
...
hidden_states_true = torch.nn.functional.scaled_dot_product_attention(
    ...
    kv_packed.view(1, 257, 2, 40, 128)[:, :, 0, :, :].transpose(1, 2),
    kv_packed.view(1, 257, 2, 40, 128)[:, :, 1, :, :].transpose(1, 2),
)
